...
fetch("app/js/navbarButtons.json")
    ...
    .then(function(navbarButtons) {
        ...
        navbarButtons.forEach(navbarButton => {
            navbarButtonElements += `<a href="${navbarButton["href"]}" class="nav__link"><span class="material-symbols-rounded">${navbarButton["linkIcon"]}</span><span class="nav__text">${navbarButton["linkName"][lang]}</span></a>`;
        });
